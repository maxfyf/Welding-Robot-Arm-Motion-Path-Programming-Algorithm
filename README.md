...
*Motion Path Planning Algorithm for Welding Robotic Arms* is a C++ project specifically designed for scientific research on the optimization of algorithms that help find ideal motion path of welding robotic arms. This project is focused on improving the ultimate result of Beam search algorithm and Rapidly-exploring Random Tree (RRT) algorithm. In other words, it is dedicated to coming up with optimized Beam search algorithm and optimized RRT algorithm that compute outputs indicating lower distance cost for a welding robotic arm, but in the meantime do not take an unacceptably long running time. Both algorithm codes and experiment codes are included in this project, and some plotting codes are involved as well.

### Project Hierarchy

Motion-Path-Planning-Algorithm-for-Welding-Robotic-Arms
...
This project requires C++ environment and basic STL libraries. You can run this project by compiling Test.cpp together with all the other files. You can also modify the main function in Test.cpp to recur the results of certain experiments as you will. If you want to call functions with visualization of the result, you need to configure the plotting file matplotlibcpp.h properly, and the header file is provided under the path '/tools'.
